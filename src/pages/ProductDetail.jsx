...
const categoryTranslations = {
    ...
    "ягоды и продукты их переработки (кроме соков)": {
        en: "berries and their processed products (except juices)",
        ru: "ягоды и продукты их переработки (кроме соков)"
    },
...
export default function ProductDetail() {
    ...
    useEffect(() => {
        ...
        const fetchDetail = fetch(`${API}/product/details/${id}?lng=${lang}`, { mode: 'cors' })
            .then(res => {
                if (!res.ok) throw new Error(`Detail: ${res.status}`);
                return res.json();
            });

        const fetchRecipe = fetch(`${API}/recipe/${id}`, { mode: 'cors' })
            .then(res => {
                if (!res.ok) throw new Error(`Recipe: ${res.status}`);
                return res.json();
            });

        // Ждем выполнения всех запросов (аналог Future.wait)
        Promise.all([fetchDetail, fetchRecipe])
            .then(([detailData, recipeData]) => {
                ...
                console.log('Рецептура (recipeData):', recipeData);

                // Сохраняем оба результата в объект
                setData({
                    detail: detailData,
                    recipe: recipeData
                });
    ...
    return (
        <div className="product-detail">
            ...
            <div className="title-container">
                <h1 className="title">{data.detail.name}</h1>
            ...
            <p className="subtitle">
                {regionTranslations[data.detail.regionName]?.[lang] || data.detail.regionName} · {categoryTranslations[data.detail.categoryName]?.[lang] || data.detail.categoryName}
            </p>
            ...
            <section>
                ...
                <table className="comp-table">
                    ...
                    <tbody>
                    {data.detail.chemicalComposition?.map(c => (
                        <tr key={c.id}>
            ...
            <section>
                ...
                <table className="comp-table">
                    ...
                    <tbody>
                    {data.detail.mineralComposition?.map(m => (
                        <tr key={m.id}>
